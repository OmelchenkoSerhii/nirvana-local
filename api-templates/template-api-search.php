...
if($eventPostID): //If event code passed - create new order
    ...
    $bookingPageType = get_field('booking_landing_page' , $eventPostID);  
...
$bookingPageType = !$bookingPageType?'packages':$bookingPageType;
	...
	<section class="booking text--color--dark">
		<?php if($order): ?>
			...
			<div class="booking__col-content px-2">
                <?php if ( ! empty( $_GET['eventid'] ) && is_celtic_active( $api->getEventIDbyCode( $_GET['eventid'] ) ) ) : ?>
                    <div class="header__info-bar align-items-center justify-content-between mx-n2 mt-n2">
                        <div class="header__info-bar__content text-center" style="color: #FFF; ">To book this event you will need to provide a valid client reference number for each supporter.</div>
                    </div>
                <?php endif; ?>    

                <?php if($bookingPageType == 'packages'): ?>
                    <?php $order->setOrderType('tour'); ?>
                ...
                <?php elseif($bookingPageType == 'accommodation'): ?>
                    <?php $order->setOrderType('tailor'); ?>
                    <?php do_action('order_content_header' , 'accomomodations' , $order , $api); ?>
                    <?php get_template_part( 'template-parts/api/accommodation/order', 'accommodations',  array(
                        'api' => $api,
                        'order' => $order,
                    )); ?>
                <?php endif; ?>
