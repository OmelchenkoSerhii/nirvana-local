...
if(isset($args['event_id']) && $args['event_id']):
    ...
    $celtic = is_celtic_active($args['event_id']);

    
    if($start_date):
        $dateFormat = DateTime::createFromFormat('Ymd', $start_date);
        if($dateFormat):
            $start_date = $dateFormat->format('F j, Y');
        endif; 
    endif;

    if($end_date):
        $dateFormat = DateTime::createFromFormat('Ymd', $end_date);
        if($dateFormat):
            $end_date = $dateFormat->format('F j, Y');
        endif;
    endif;
    ...
    if($celtic):
        $redirect_link = get_field('api_packages_page','option');
    else:
        $redirect_link = get_field('api_booking_type_page','option');
    endif;
    ?>

    <?php if(false): ?>
        <pre><?php api_sync_events(); ?></pre>
    <?php endif; ?>
    
    <div class="order-dates">
        <form method="GET" action="<?php echo $redirect_link; ?>" id="order-dates" class="order-form">
            ...
            <div class="order-form__row align-items-end">
                <div  class="datepicker datepicker--not-selected" data-start="<?php echo $start_date; ?>" data-end="<?php echo $end_date; ?>">
                    <span class="datepicker__field order-form__field order-form__field--checkin">
                        ...
                        <span class="datepicker__field-inner">
                            <input type="text" name="date-checkin" required readonly placeholder="<?php _e('Select Date' , 'nirvana'); ?>"/>
                    ...
                    <span class="datepicker__field order-form__field order-form__field--checkout">
                        ...
                        <span class="datepicker__field-inner">
                            <input type="text" name="date-checkout" required readonly placeholder="<?php _e('Select Date' , 'nirvana'); ?>"/>
                            <?php echo get_inline_svg('icons/icon-calendar.svg'); ?>
                        </span>
                    </span>
                                        ...
                                        <div class="people-field__qtt">
                                            <div class="people-field__qtt-inner">
                                                <span class="people-field__qtt__label">
                                                    ...
                                                    <span class="small"><?php esc_html_e('Ages 0-17', 'nirvana'); ?></span>
                                                </span>
                                                <span class="people-field__qtt__select">
            ...
            <input type="hidden" name="redirect_url" value="<?php echo $redirect_link; ?>?order_token=<?php echo $order_token; ?>">
        </form>
    </div>
...
<?php else: ?>
    <h3><?php esc_html_e('Please select the event','nirvana'); ?></h3>
<?php endif; ?>
